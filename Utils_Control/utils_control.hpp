   ...
   field<cube>  rlqr_slsm(cube F, cube Ef, cube G, cube Eg, cube H, cube P, cube Q, cube R, cube M, int s, mat prob, int N){
      ...
      for(int k=0;k<=N-2;k++){ //N
            ...
            for(int i=0;i<=(s-1);i++){
               ...
               P_modo = P.slice(i); 
               P_modo.submat(0, k+1,(n*n-1),k+1) = arma::reshape(UW,n*n,1); 
               P.slice(i) = P_modo;

               //P.slice(i) = reshape(UW,n*n,1,0);
               
               VU = V.t()*inv(W)*U;
               
               K_modo = K.slice(i); 
               K_modo.submat(0,k,(n*m)-1,k) = arma::reshape(VU,n*m,1); 
               //reshape(cube, n_rows, n_cols, n_slices, dim=0)
               K.slice(i) = K_modo;
               ...
               L_modo = L.slice(i); 
               L_modo.submat(0, k, (n*n)-1, k) = arma::reshape(ZW,n*n,1); 
               L.slice(i) = L_modo;
